...
char *window_title = "移动的星星";
GLfloat z = -5.0f;

BOOL twinkle; //闪烁的星星
BOOL tp;      // T键按下

const int length = 5; //纹理个数
const int num = 50;   //星星个数
...
void LoadTexture()
{
    ...
    if (texture[4].loadTexture("./Data/moon.jpg"))
    {
        //月亮
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture[4].width, texture[4].height, 0, GL_RGB, GL_UNSIGNED_BYTE, texture[4].data);
    }
}
...
int InitGL(GLvoid)
{
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE); //设置混合模式，半透明
...
int DrawGLScene(GLvoid)
{
    ...
    //清除颜色缓存和深度缓存,重置场景
    glBindTexture(GL_TEXTURE_2D, texture[4].id); //绑定纹理
    ...
    for (loop = 0; loop < num; loop++)
    {
        ...
        glRotatef(star[loop].angle, 0.0f, 1.0f, 0.0f);  //旋转至当前所画星星角度
        glTranslatef(star[loop].dist, 0.0f, 0.0f);      //移动至当前所画星星距离
        glRotatef(-star[loop].angle, 0.0f, 1.0f, 0.0f); //取消当前所画星星角度
        glRotatef(-tilt, 1.0f, 0.0f, 0.0f);             //取消倾斜视角
        if (twinkle)
        {

...
void SetupWorld(){
    FILE *fp;
    fp = fopen(worldfile, "rt");
    int numtriangles;
    char oneline[255];
    float x, y, z, u, v;

    readstr(fp, oneline);
    sscanf(oneline, "numtriangles %d", &numtriangles);
    sector1.triangle = new TRIANGLE[numtriangles];
    sector1.numtriangles = numtriangles;
    for (int i = 0; i < numtriangles; i++)
    {
        readstr(fp, oneline);
        sscanf(oneline, "vertex %f %f %f %f %f", &x, &y, &z, &u, &v);
        sector1.triangle[i].vertex[0].x = x;
        sector1.triangle[i].vertex[0].y = y;
        sector1.triangle[i].vertex[0].z = z;
        sector1.triangle[i].vertex[0].u = u;
        sector1.triangle[i].vertex[0].v = v;
        readstr(fp, oneline);
        sscanf(oneline, "vertex %f %f %f %f %f", &x, &y, &z, &u, &v);
        sector1.triangle[i].vertex[1].x = x;
        sector1.triangle[i].vertex[1].y = y;
        sector1.triangle[i].vertex[1].z = z;
        sector1.triangle[i].vertex[1].u = u;
    fclose(fp);
}
            ...
            if(active){
                if(keys[VK_ESCAPE]){
                    ...
                    if(keys[VK_UP]){
                        ...
                        walkbiascos = (float)sin(walkbiasangle * piover180)/20.0f;
                    }
                    if(keys[VK_DOWN]){
                        xpos += (float)sin(heading * piover180) * 0.05f;
                        zpos += (float)cos(heading * piover180) * 0.05f;

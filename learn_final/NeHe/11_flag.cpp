...
void LoadTexture(){
    //加载纹理
    if(texture[0].loadTexture("./Data/Tim.bmp")){
        //设置纹理属性
...
int InitGL(GLvoid){
    ...
    glPolygonMode(GL_FRONT, GL_LINE);//前表面只有线框
    for (int x = 0;x<45;x++){
        for(int y = 0;y<45;y++){
            points[x][y][0] = float((x / 5.0f) - 4.5f);
            points[x][y][1] = float((y / 5.0f) - 4.5f);
            points[x][y][2] = float(sin((((x/5.0f)*40.0f)/360.0f)*3.1415926*2.0f));
        }
    }
    return TRUE;
}
...
int DrawGLScene(GLvoid){
    ...
    if(wiggle_count==100){
        for (y = 0; y < 45;y++){
            hold=points[0][y][2];//储存当前左侧波浪值
            for(x = 0;x < 44;x++){
                points[x][y][2] = points[x+1][y][2];//将当前波浪值移动到右侧
            }
            points[44][y][2] = hold;//将储存的值移动到左侧
        }
        wiggle_count = 0;
    }
    wiggle_count++;
